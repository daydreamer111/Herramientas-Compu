...
def convolution(image, kernel, average=False, verbose=False):
    if len(image.shape) == 3: #imagen a colores detectada
        print("Found 3 Channels : {}".format(image.shape))
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #cambiando a blanco y negro (disminuir de 3 a 2 dimensiones)
